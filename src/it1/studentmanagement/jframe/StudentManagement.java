...
public class StudentManagement {
	...
	private void initializeStudent() {
		...
		studentSearchPanel.setBounds(818, 261, 310, 293);
		...
		btnEditStudent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (table.getSelectedRow() == -1) {
					JOptionPane.showMessageDialog(null, "You have to choose the student row first", "Lỗi", JOptionPane.ERROR_MESSAGE);
				} else {
		...
		btnSaveStudent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (name.isEmpty() || dob.isEmpty()|| txtMath.getText().isEmpty() 
						|| txtPhysical.getText().isEmpty() || txtChemistry.getText().isEmpty()
						|| gender == -1 || province == null){
					showMessage("Vui lòng điền đầy đủ thông tin!", JOptionPane.ERROR_MESSAGE);
					return;
				}
				float math = -1;
				float physical = -1;
				...
				try {
					...
					showMessage("Điểm không hợp lệ!", JOptionPane.ERROR_MESSAGE);
					return;
				}
				//TH điểm k hợp lệ
				if (math < 0 || math > 10 ||  physical < 0 || physical > 10 || chemistry < 0 || chemistry > 10 ){
					showMessage("Điểm phải từ 0 -10!", JOptionPane.ERROR_MESSAGE);
					return;
				}
				
				student = new StudentDTO(id, name,province, dob, gender, math,physical, chemistry);
				
				//bắt lỗi nhập ngày sinh k hợp lệ
				if (!isEditStudent) {
		...
		btnDeleteStudent.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (table.getSelectedRow() == -1) {
					...
					int accept =  JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa?\nID: " + table.getValueAt(selectedRow, 0).toString() + "\nName: " + table.getValueAt(selectedRow, 1).toString() , "Xóa thí sinh", JOptionPane.YES_NO_OPTION);
					if (accept == JOptionPane.YES_OPTION) {
						String message = studentsBUS.delete(Integer.parseInt(table.getValueAt(selectedRow, 0).toString()));
		...
		btnSearch.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				List<StudentDTO> studentsSearchList = studentsBUS.getStudentList(studentIdSearch, studentProvinceNameSearch);
		...
		btnSaveProvince.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (txtProvinceName.getText().isEmpty()) {
					showMessage("Province name can't be null", JOptionPane.ERROR_MESSAGE);
		...
		btnDeleteProvince.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selectedRowIndex != -1) {
					int accept =  JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa?\nID: " + provinceTable.getValueAt(selectedRowIndex, 0).toString() + "\nName: " + provinceTable.getValueAt(selectedRowIndex, 1).toString() , "Xóa tỉnh", JOptionPane.YES_NO_OPTION);
					if (accept == JOptionPane.YES_OPTION) {
					...
					JOptionPane.showMessageDialog(null, "You have to choose a province row first", "Lỗi", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
	...
	private void setEditableTextFieldStudent(boolean bool) {
		comboBoxStudent.setEditable(bool);
